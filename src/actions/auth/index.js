...
import { apiDomain } from "../../utils/domainRoute";
...
export const signUp = ({ username, password }) => {
  return (dispatch) => {
    ...
    return axios
      .post(`${apiDomain}/api/users/signup`, { username, password })
      .then((res) => {
...
export const signIn = ({ username, password }) => {
  return (dispatch) => {
    dispatch(setOnoingServerActivity({ authenticating: true }));
    return axios
      .post(`${apiDomain}/api/users/signin`, { username, password })
      .then((res) => {
...
export const loadUserByToken = () => {
  return (dispatch, getState) => {
    ...
    if (config.headers["x-auth-token"]) {
      dispatch(setOnoingServerActivity({ authenticating: true }));
      return axios
        .get(`${apiDomain}/api/users/user`, config)
        .then((res) => {
...
export const editPassword = ({ newPassword, oldPassword }) => {
  return (dispatch, getState) => {
    if (newPassword && oldPassword) {
      ...
      return axios
        .patch(
          `${apiDomain}/api/users/edit-password`,
...
export const deleteUser = (password) => {
  return (dispatch, getState) => {
    if (password) {
      ...
      return axios
        .delete(`${apiDomain}/api/users/delete-user`, {
          ...tokenConfig(getState().auth.token),
          data: { password },
        })
        .then((res) => {
